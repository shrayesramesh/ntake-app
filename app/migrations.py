...
``app.db.Base.metadata`` for autogenerate. Keeping the ``Config`` here (not an
ini) means one source of truth for the URL: ``CALENDAR_DB_URL`` via ``app.db``.
"""
...
def make_alembic_config(url: str | None = None) -> Config:
    ...
    ``CALENDAR_DB_URL`` (via ``app.db.DB_URL``). Sets ``script_location`` to the
