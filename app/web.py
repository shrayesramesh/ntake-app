...
<head>
  ...
  <style>
    ...
    #capture { display: flex; gap: .5rem; margin-bottom: 1rem; }
...
<body>
  ...
  <form id="capture"
        hx-post="/work-items/capture"
        hx-target="#board-container"
        hx-swap="innerHTML"
        hx-on::after-request="if(event.detail.successful) this.reset();">
    <input name="title" placeholder="Add something to the board…" required>
  ...
  <script>
    ...
    // Attach the bearer token to every HTMX request. (The capture form posts
    // form-encoded to /work-items/capture — no JSON rewrite needed.)
    ...
    function reloadBoard() {
      ...
      fetch('/board/view', { headers: { 'Authorization': 'Bearer ' + t } })
