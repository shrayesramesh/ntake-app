...
# testing (the persistent-DB, real-household counterpart to `make smoke`).
#
...
#   * THIS script: the PERSISTENT ./calendar.db, YOUR real family.toml household,
#     and the LIVE local LLM (kind=local via NTAKE_ASSISTANT_KIND). It seeds
#     sample events, mints a device token, and serves so you can drive real
#     captures in a browser and watch genuine model reasoning + the debug panel.
#
# PREREQUISITE (the other/infra session owns this): the llamafile model server
# must already be answering on the configured base URL. Bring it up with
# `make llm-up` first. This script does NOT start the model.
#
# Usage:
#   scripts/live_llm_ui.sh            # preflight, seed, mint, flip, serve
...
#
# OUT-OF-REPO FILES THIS SCRIPT MAY CREATE (both under the config dir, gitignored
# by location; never in the repo):
#   <config-dir>/family.toml     scaffolded from family.example.toml if missing
#   <config-dir>/token_secret    a stable secret (chmod 600) if none is set
...
# --- 0. config + token secret (scaffold if missing) -----------------------
...
# --- 4. seed sample events (idempotent-ish; adds a couple each run) --------
...
==================================================================
 Live-LLM UI session is UP.
   ...
   DB:     ./calendar.db (persistent)   log: $LOG
